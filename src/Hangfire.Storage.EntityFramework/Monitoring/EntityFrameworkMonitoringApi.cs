...
namespace Hangfire.Storage.EntityFramework.Monitoring
{
    ...
    {
        public static JobList<TDto> ToJobList<TDto>(this IEnumerable<Entities.Job> jobList, Func<Entities.Job, TDto> expr)
        {
            return new JobList<TDto>(jobList.Select(e => new KeyValuePair<string, TDto>(e.Id.ToString(), expr.Invoke(e))));
        }
    }
    ...
    internal class EntityFrameworkMonitoringApi : IMonitoringApi
    {
        ...
        public EntityFrameworkMonitoringApi(HangfireDbContext dbContext, IOptions<EntityFrameworkStorageConfiguration> options)
        {
            _dbContext = dbContext;
            _options = options;
        }

        private object DeserializeDataKey(string key, string data)
        {
        ...
        public JobList<DeletedJobDto> DeletedJobs(int from, int count)
        {
            return _dbContext.Jobs
                ...
                .ToJobList(e=>
                    new DeletedJobDto
                    {
                        DeletedAt = DeserializeDataKey("DeletedAt", e.State.Data) as DateTime?
                    }); 
        }

        public long DeletedListCount()
        {
            return _dbContext.Jobs.Where(e => e.StateName == DeletedState.StateName).Count();
        }
...
private T UseConnection<T>(Func<EntityFrameworkConnection, T> action) => 
   _storage.UseConnection(action);

private long GetNumberOfJobsByStateName(EntityFrameworkConnection connection, string stateName)
{
...
private JobList<TDto> GetJobs<TDto>(
   EntityFrameworkConnection connection,
   int from,
   int count,
   string stateName,
...
private Dictionary<DateTime, long> GetTimelineStats(EntityFrameworkConnection connection,
...
private JobList<EnqueuedJobDto> EnqueuedJobs(
   EntityFrameworkConnection connection, int[] ids)
{
